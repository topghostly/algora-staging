...
function GroupByJourney({platform}: any){
  ...
  const getKey=(r: any)=>String(r[groupCol]);
  const groups=[...new Set(EMP.map(getKey))].sort();
  const grouped: any={};groups.forEach(g=>{grouped[g]=EMP.filter(r=>getKey(r)===g);});
                    ...
                    <div key={g} style={{border:`1px solid ${col.border}`,borderRadius:8,overflow:"hidden",animation:`fadeUp .3s ease ${gi*70}ms both`}}>
                      ...
                      <div style={{padding:"5px 10px",background:col.bg,borderTop:`1px dashed ${col.border}`,fontSize:9,color:col.text,fontFamily:"monospace"}}>
                        ...
                        {aggFn==="COUNT"?` ${rows.length}`:
                         aggFn==="SUM"&&aggCol!=="*"?` ${rows.map((r: any)=>r[aggCol]).filter(Boolean).join("+")} = ${val}`:
                         aggFn==="AVG"&&aggCol!=="*"?` avg(${rows.map((r: any)=>r[aggCol]).filter(Boolean).join(",")}) ≈ ${val}`:
                         aggFn==="MIN"&&aggCol!=="*"?` min(${rows.map((r: any)=>r[aggCol]).filter(Boolean).join(",")}) = ${val}`:
                         aggFn==="MAX"&&aggCol!=="*"?` max(${rows.map((r: any)=>r[aggCol]).filter(Boolean).join(",")}) = ${val}`:` ${val}`}
                      </div>
                    </div>
                  );
                })}
...
function HavingVisual({platform}: any){
  const pc=PLAT[platform as keyof typeof PLAT].color;
  const [fn,setFn]=useState("COUNT");
  const [col,setCol]=useState("*");
  const [thresh,setThresh]=useState(2);
  const groups=["Engineering","Analytics","Product"];
  const grouped: any={};groups.forEach(g=>{grouped[g]=EMP.filter(r=>r.dept===g);});
  const computeVal=(rows: any[])=>{
...
function AggFnHover({platform}: any){
  const pc=PLAT[platform as keyof typeof PLAT].color;
...
export default function Module03({platform}: any){
  const pc=PLAT[platform as keyof typeof PLAT].color;
