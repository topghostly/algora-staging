...
export default function QuizViewer({ lessonId, initialCompleted }: QuizViewerProps) {
    ...
    if (questions.length === 0) return <div style={{ padding: "2rem", textAlign: "center", color: "var(--muted)" }}>No questions in this quiz yet.</div>;
            ...
            {!submitted ? (
                <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
                                ...
                                {q.options.map((option) => (
                                    <label
                                        ...
                                        style={{
                                            ...
                                            border: answers[q.id] === option.id ? "1px solid var(--primary)" : "1px solid var(--border)",
                                            borderRadius: "var(--radius)",
                                            backgroundColor: answers[q.id] === option.id ? "rgba(var(--primary-rgb), 0.05)" : "transparent",
                                            cursor: "pointer",
                                            transition: "all 0.2s"
                                        }}
                                    >
                    ...
                    <button
                        ...
                        style={{ alignSelf: "flex-start", padding: "0.75rem 2rem", fontSize: "1rem" }}
                    >
                        Submit Quiz
                    </button>
                </div>
                ...
                <div className="card" style={{ padding: "3rem", textAlign: "center" }}>
                    <div style={{ marginBottom: "1.5rem" }}>
                        {passed ? (
                            <CheckCircle size={64} color="#16a34a" style={{ margin: "0 auto", marginBottom: "1rem" }} />
                        ) : (
                            <XCircle size={64} color="#ef4444" style={{ margin: "0 auto", marginBottom: "1rem" }} />
                        )}
                        <h2 style={{ fontSize: "2rem", fontWeight: 700, marginBottom: "0.5rem" }}>
                            {passed ? "Quiz Passed!" : "Quiz Failed"}
                        </h2>
                        <p style={{ fontSize: "1.2rem", color: "var(--muted)" }}>
                            You scored <span style={{ fontWeight: 700, color: "var(--foreground)" }}>{score}%</span>
                        </p>
                    </div>
                    ...
                    <div style={{ display: "flex", justifyContent: "center", gap: "1rem" }}>
                        {!passed ? (
                            <button onClick={handleRetry} className="btn btn-primary">
                                <RefreshCw size={18} style={{ marginRight: "0.5rem" }} /> Retry Quiz
                            </button>
                        ) : (
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem" }}>
                                <p style={{ color: "var(--muted)" }}>Great job! You can now complete this lesson.</p>
                                <LessonCompleteButton lessonId={lessonId} initialCompleted={initialCompleted} />
                            </div>
                        )}
                    </div>
                </div>
