            ...
            {bookings.length === 0 ? (
                ...
                <div className="space-y-4">
                    {bookings.map((booking) => (
                        <div key={booking.id} className="border-b border-border pb-4 last:border-0 last:pb-0">
                            <h3 className="font-semibold">{booking.session.title}</h3>
                            <p className="text-sm text-muted-foreground mb-2">with {booking.session.tutor.name}</p>
